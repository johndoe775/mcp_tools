...
def resume_rewrite(resume,job_description,personal_info=""):
    ...
    response=tool_maker(resume,job_description,personal_info).llm_fx(list(data.keys())[0])
    return response 

@mcp.tool
def cold_mail_info_present(resume,personal_info,job_description="",):
    """ use this tool to write a cold mail to a recruiter when you have personal info about them """
    response=tool_maker(resume,job_description,personal_info).llm_fx(list(data.keys())[1])
    return response
...
def cold_mail_with_no_info(resume,job_description="",personal_info=""):
    ...
    response=tool_maker(resume,job_description,personal_info).llm_fx(list(data.keys())[2])
    return response
